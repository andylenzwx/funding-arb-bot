...
async def _retry_call(operation: Callable[[], Awaitable[T]]) -> T:
    return await operation()


async def retry_api_call(operation: Callable[[], Awaitable[T]]) -> T:
    """Retry wrapper for API calls with exponential backoff.

    Args:
        operation: Zero-argument callable that produces the coroutine to execute
    ...
    return await _retry_call(operation)
